...
<div class="form-horizontal additional-bordered">
    ...
    <p class="note">Have you already submitted data to a public repository that 
    is directly described as part of this dataset? E.g. raw sequence data 
    submitted to the Sequence Read Archives.</p>
    ...
    <div style="text-align: center; margin-bottom: 15px;">
        ...
        <a href="#"
           ...
           id="public-links-yes"
           class="btn btn-default additional-button <?php if ($isPublicLinks === true): ?>btn-success disabled <?php else: ?>js-yes-button<?php endif; ?>"/>Yes</a>
        ...
        <a href="#"
           ...
           id="public-links-no"
           data-id="<?= $model->id ?>"
           class="btn btn-default additional-button <?php if ($isPublicLinks === false): ?>btn-success disabled <?php else: ?>js-no-button<?php endif; ?>"/>No</a>
    </div>
    ...
    <div id="public-links"<?php if ($isPublicLinks !== true): ?> style="display: none"<?php endif; ?>>
        <div class="row subwiz-row">
            <div class="col-xs-12">
                <p class="note">Please select the appropriate database from the 
                list (you may repeat process to add different database links).</p>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <?= CHtml::dropDownList('prefix', null, array('' => 'Please select') + CHtml::listData($link_database,'prefix','prefix'), array('class'=>'js-database form-control', 'style'=>'width:250px')); ?>
                    </div>
                </div>
            </div>
        </div>
        <div class="row subwiz-row">
            <div class="col-xs-12">
                <p class="note js-set-database">Please select add accession 
                numbers to your data in the above database</p>
                            ...
                            <?php foreach($links as $link): ?>
                                ...
                                <tr class="odd js-my-item">
                                    ...
                                    <td class="button-column">
                                        ...
                                        <a href="#" id="delete-link" class="js-delete-link delete-title" link-id="<?=$link->id?>" data-id="<?= $model->id ?>" title="delete this row">
            ...
            success: function(response){
                if(response.success) {
                    ...
                    var tr = '<tr class="odd js-my-item">' +
                        ...
                        '<td class="button-column">' +
                        '<a href="#" id="delete-link" class="js-delete-link delete-title" title="delete this row">' +
                        '<img alt="delete this row" src="/images/delete.png">' +
                        '</a>' +
                        '</td>' +
                        '</tr>';
